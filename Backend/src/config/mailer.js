...
const buildItemsTable = (orderItems) => {
    return orderItems
      .map(
        ...
        ${
          it.size
            ? `<div style="color:#777; font-size:12px;">Size: ${it.size}</div>`
            : ""
        }
        </td>
        <td style="padding:8px 10px; border-bottom:1px solid #e9e9e9; text-align:center; font-size:14px;">${it.quantity}</td>
        <td style="padding:8px 10px; border-bottom:1px solid #e9e9e9; text-align:right; font-size:14px;">${formatCurrency(
        it.product.price
        )}</td>
        </tr>`
      )
...
const sendOrderConfirmationEmail = async (to, order) => {
  ...
  const itemsTableRows = await buildItemsTable(order.orderItems || []);
