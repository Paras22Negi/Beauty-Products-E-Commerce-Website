...
const sendEmail = async (email, otp) => {
  try {
    await transporter.sendMail({
      from: `"MARS Cosmetics Clone By -Paras Negi" <${process.env.FROM_EMAIL}>`,
      to: email,
      subject: "Your OTP for MARS Cosmetics Account Verification",
      html: `
        <div style="font-family: Arial, sans-serif; padding: 20px;">
          <h2 style="color: #5c4dff;">Welcome to MARS Cosmetics Clone!</h2>
          ...
          <p style="color: #aaa;">— Team MARS Cosmetics Clone</p>
        </div>
      `,
    });
